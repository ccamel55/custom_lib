...
struct vtable {
    ...
    constexpr vtable(const address vtable_ptr_address) {
        ...
        for (size = 0; vtable[size]; size++) {
            ...
            if (static_cast<unsigned long>(vtable[size]) >> 16 == 0) {
                break;
            }
        }

        const std::span table_span(reinterpret_cast<std::byte*>(vtable), size * sizeof(uintptr_t));
    ...
    [[nodiscard]] constexpr std::optional<address> get(const size_t index) const {
        if (index >= size()) {
            return std::nullopt;
        }
