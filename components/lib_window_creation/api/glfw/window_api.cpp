...
#if !defined(DEF_LIB_INPUT_off) && !defined(DEF_LIB_INPUT_glfw)
// if we are using an input backend that doesn't match our window creator then need to assert
static_assert(false, "lib_input: input backend does not match window creation backend");
#endif
