...
namespace lib
{
	...
	inline std::vector<std::string> SPLIT_STRING(const std::string& string, const char split = ' ')
	{
		...
		while ((second_idx = string.substr(first_idx, std::string::npos).find(split)) != std::string::npos)
		{
			...
			// second_idx should be at the index of the split character, therefore substring should not include split character
			// if split character is included then it will be size 0 or empty, thus we skip.
			if (!sub_string.empty())
			{
