...
vertex_array_object::vertex_array_object(std::unique_ptr<vertex_buffer_object> vertex_buffer_object, const vertex_layout& vertex_layout) : _vertex_array_object(0), _vertex_buffer_object(std::move(vertex_buffer_object))
{
	...
	for (size_t i = 0; i < layout.size(); i++)
	{
		...
		glVertexAttribPointer(i, curParam.count, curParam.type, curParam.normalized, vertex_layout.get_stride(), reinterpret_cast<void*>(offset));
